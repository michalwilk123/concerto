...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const ancestors = await db.execute(sql`
    WITH RECURSIVE chain AS (
      SELECT id, name, group_id, parent_id, meeting_id, created_at, 0 AS depth
      FROM folder
      WHERE id = ${id}
      UNION ALL
      SELECT f.id, f.name, f.group_id, f.parent_id, f.meeting_id, f.created_at, c.depth + 1
      FROM folder f
      JOIN chain c ON f.id = c.parent_id
      WHERE c.depth < 50
    )
    SELECT id, name, group_id AS "groupId", parent_id AS "parentId",
           meeting_id AS "meetingId", created_at AS "createdAt"
    FROM chain
    ORDER BY depth DESC
  `);
