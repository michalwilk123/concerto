...
/**
 * Create a RealtimeKit participant and store the participant mapping.
 * Also tracks teachers in room.connectedTeachers.
 */
export async function createRealtimeKitParticipant(params: {
  ...
  const customParticipantId = nanoid();
  const { id, token } = await addParticipant(
    rtkMeetingId,
    participantName,
    preset,
    customParticipantId,
  );
  room.participants.set(participantName, { rtkId: id, customParticipantId, role });
  if (role === "teacher") {
